...
int main()
{
    ...
    Shader shader("Assets/shader/advanced_glsl/4_8_shader.vs", "Assets/shader/advanced_glsl/4_8_shader.fs");
    ...
    // 绑定uniform中对应的绑定点
    //unsigned int uniformIndexMatrices = glGetUniformBlockIndex(shader.ID, "Matrices");
    //glUniformBlockBinding(shader.ID, uniformIndexMatrices, 0);
    //uniformIndexMatrices = glGetUniformBlockIndex(skyboxShader.ID, "Matrices");
    //glUniformBlockBinding(skyboxShader.ID, uniformIndexMatrices, 0);
    ...
    while (!glfwWindowShouldClose(Viewport::Instance()->GetWindow()))
    {
        ...
        skyboxShader.SetMat4("projection", _perspective_projection_matrix);
        glBindVertexArray(skyboxVAO);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_CUBE_MAP, skybox.ID);
        glDrawArrays(GL_TRIANGLES, 0, 36);
